...
Mỗi service đều tuân theo cấu trúc trên với các package tương ứng.

## Service Communication
- **Đồng bộ**: sử dụng **OpenFeign** (REST) và **gRPC** (hiệu suất cao, dùng cho Order ↔ Payment).
- **Bất đồng bộ**: dùng **Apache Kafka** cho các sự kiện lớn (ví dụ: product.created → đồng bộ search, analytics), và **RabbitMQ** cho tác vụ ngắn (gửi email, SMS).